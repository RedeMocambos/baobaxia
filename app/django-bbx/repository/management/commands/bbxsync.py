...
class Command(BaseCommand):
    """Sincroniza a mucua local, a partir dos metadados e configurações do
    repositório."""
    help = 'Create media objects from new serialized objects.'
    args = '[repository name] [repository name] ...'

    def add_arguments(self, parser):
        parser.add_argument('repository', type=str, help='Nome do repositorio')
...
#        for repository in args:
        ...
        output = subprocess.check_output(
            ...
            cwd=path)
        logger.debug("%s: %s" % (_('Revision is'), output))
        logger.info('<<<')
        last_sync_mark = open(os.path.join(path, 'lastSync.txt'), 'w+')
        last_sync_mark.write(output.decode())
