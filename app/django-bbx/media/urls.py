...
urlpatterns = patterns('media.views',
    ...
    url(r'^(?P<repository>\w+)/(?P<mucua>[a-zA-Z0-9\-]+)/media/last/(?P<qtd>[\d]*)$', 'media_last'),
    url(r'^(?P<repository>\w+)/(?P<mucua>[a-zA-Z0-9\-]+)/media/last', 'media_last'),
    url(r'^(?P<repository>\w+)/(?P<mucua>rede)/bbx/search/(?P<args>[\w\/-_]*)$', 'media_list'),
    url(r'^(?P<repository>\w+)/(?P<mucua>[a-zA-Z0-9\-]+)/bbx/search/(?P<args>[\w\/-_]*)$', 'media_list'),
    url(r'^(?P<repository>\w+)/(?P<mucua>[a-zA-Z0-9\-]+)/media/(?P<pk>[\w\-]*)$', 'media_detail'),
